...
def main(cfg: TrainConfig) -> None:
    ...
    if cfg.wandb is not None and (get_global_rank() == 0 or not cfg.wandb.rank_zero_only):
        if cfg.wandb_save_folder is None:
            wandb_dir = Path(cfg.save_folder) / "wandb"
        else:
            wandb_dir = Path(cfg.wandb_save_folder)
    ...
    olmo_model = OLMo(cfg.model, 
                      cfg.quant_nvfp4) # Replace Linear Layers when building, rather than replace later
    if cfg.quant_nvfp4 is not None and cfg.quant_nvfp4.enable_nvfp4_training:
        log.info("Enabled NVFP4 Training")
        log.info(f"- Enabled NVFP4 Layers: {cfg.quant_nvfp4.enabled_layers}")
        log.info(f"- Enabled NVFP4 Quantizers: {cfg.quant_nvfp4.enabled_quantizers}")
        log.info(f"- Double Blocksize: ({cfg.quant_nvfp4.outer_blocksize}, {cfg.quant_nvfp4.inner_blocksize})")
