            ...
            with FSDP.state_dict_type(
                ...
                with torch.no_grad():
                    ...
                    for module_name, module in dist_model.named_modules():
                        ...
                        # Load Buffers
                        for buf_name, buf in module._fsdp_wrapped_module.named_buffers():
                            if buf is None:
                                continue
                            
                            key = f"{module_name}.{buf_name}"
                            key = key.replace("_fsdp_wrapped_module.", "")
                            key = key.lstrip(".")
                            
                            if key not in state_dict_to_load:
                                continue
                            
                            if state_dict_to_load[key].shape == torch.Size([]):
                                log.info(f'buffer loaded: {buf_name} {buf.dtype} <- {state_dict_to_load[key]}')
                            else:
                                log.info(f'buffer loaded: {buf_name} {buf.dtype} <- {state_dict_to_load[key].dtype, state_dict_to_load[key].shape}')
                            t = state_dict_to_load[key]
        ...
        *,
        ...
        with self._temporary_wd(dir) as checkpoint_dir:
            ...
            if get_fs_local_rank() == 0:
                ...
                print(f"created for {checkpoint_dir}")

            barrier()

            wait_for(
                lambda: (checkpoint_dir / "model").exists(), 
                f"Waiting for checkpoint model directory {checkpoint_dir / "model"}, fs_local_rank={get_fs_local_rank()}",
                timeout=10.0
            )

            wait_for(
                lambda: (checkpoint_dir / "optim").exists(), 
                f"Waiting for checkpoint optim directory {checkpoint_dir / "optim"}, fs_local_rank={get_fs_local_rank()}", 
                timeout=10.0
            )

            wait_for(
                lambda: (checkpoint_dir / "train").exists(), 
                f"Waiting for checkpoint train directory {checkpoint_dir / "train"}, fs_local_rank={get_fs_local_rank()}", 
                timeout=10.0
            )
