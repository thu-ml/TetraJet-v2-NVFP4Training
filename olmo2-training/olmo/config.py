...
@dataclass
class NVFP4_Outlier_Selection_Config(BaseConfig): 
    outlier_fp8_matmul: bool = True
    
    # X in fwd & bwd (select by channels)
    X_chan_select: bool = False
    X_chan_select_bwd: bool = False
    
    X_top_channels_ratio: float = 0.02
    X_chan_select_mode: str = "norm2_record" # in ['norm2_record']
    X_chan_record_start_iter: int = 1
    X_chan_record_iters: int = 10

@dataclass
class OscillationPerturb_Config(BaseConfig):
    apply_perturb: bool = False
    
    perturb_start_iter: int = 0
    perturb_period: int = 1000
    
    detect_type: str = 'distance_ratio'
    # in ['bin_edge', 'distance_ratio', 'distance_ratio_memeff']
    
    detect_BinEdge_thrd: float = 0.495 # |W_latent - W_fp4| > binwidth * thrd
    detect_DistRatio_iters: int = 50   # should be less than 'perturb_period'
    detect_DistRatio_thrd:  float = 16 # Dist_Q(w) / Dist_FP(w) >= thrd
    detect_DistRatio_sample_percent: float = 10.0 # for distance_ratio_memeff

@dataclass
class NVFP4_QuantizerConfig(BaseConfig):
    # Global Config
    enable_nvfp4_training: bool = False
    enable_qweight_buffer_when_real_quantize: bool = False # TODO
    
    # Quantization Block Settings
    inner_blocksize: int = 16
    """
    uE4M3_nv scale for inner block
    """
    
    outer_blocksize: int = 128
    """
    FP32 scale for outer block
    use `-1` to use per-tensor outer block
    """
    
    # Quantization Choice
    enabled_layers: List[str] = field(default_factory=lambda: ['all'])
    """
    enabled layers:
    - "all": all linears in transformer blocks
    - "mlp_fc1"
    - "mlp_fc2"
    - "attn_proj"
    - "attn_qkv"
    ...
    enabled_quantizers: List[int] = field(default_factory=lambda: [1, 2, 3, 4, 5, 6])
    """
    enabled quantizers:
    - 1: X in forward
    - 2: W in forward
    ...
    backward_double_quant: bool = True
    backward_stochastic: bool = True
    
    osci_perturb_setting: OscillationPerturb_Config = field(default_factory=OscillationPerturb_Config)
    outlier_conf: NVFP4_Outlier_Selection_Config = field(default_factory=NVFP4_Outlier_Selection_Config)
    
    hadamard_bwd: bool = False
    hadamard_for_dX_if_bwd: bool = True
    hadamard_for_dW_if_bwd: bool = True
    
    def is_layer_enabled(self, layer_type):
        def list_has_common_element(list1, list2):
            set1 = set(list1)
            set2 = set(list2)
            return len(set1.intersection(set2)) > 0
        
        if not self.enable_nvfp4_training:
            return False
        return layer_type != '' and \
            list_has_common_element(self.enabled_layers, {
...
class TrainConfig(BaseConfig):
    ...
    quant_nvfp4: NVFP4_QuantizerConfig = field(default_factory=NVFP4_QuantizerConfig)
    ...
    """
    wandb_save_folder: Optional[str] = None
    """
    The directory to save wandb data to. (default: None -> means = save_folder / 'wandb')
    """
