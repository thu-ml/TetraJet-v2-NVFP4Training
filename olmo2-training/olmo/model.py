...
from .config import (
    ...
    TrainConfig,
    NVFP4_QuantizerConfig
)
from .quantization_real import (
    NVFP4_mix_MXFP8_Linear_RealQuant_WeightQuantOnline,
    NVFP4_Linear_RealQuant_WeightQuantOnline
)
def NVFP4_Linear_Type(q_cfg: NVFP4_QuantizerConfig):
    if q_cfg.outlier_conf.X_chan_select:
        return NVFP4_mix_MXFP8_Linear_RealQuant_WeightQuantOnline
    else:
        return NVFP4_Linear_RealQuant_WeightQuantOnline
...
class OLMoBlock(nn.Module):
    ...
    def __init__(self, layer_id: int, config: ModelConfig, 
        ...
        ) if not q_cfg.is_layer_enabled("attn_proj") else \
        NVFP4_Linear_Type(q_cfg)(
            config.d_model, config.d_model, bias=config.include_bias, device=config.init_device,
            q_cfg=q_cfg, layer_type="attn_proj"
        )
        ...
        ) if not q_cfg.is_layer_enabled("mlp_fc2") else \
        NVFP4_Linear_Type(q_cfg)(
            int(self.act.output_multiplier * self.hidden_size),
            config.d_model,
            bias=config.include_bias,
            device=config.init_device,
            q_cfg=q_cfg, layer_type="mlp_fc2"
        )
    ...
    def build(cls, layer_id: int, config: ModelConfig, 
              q_cfg: NVFP4_QuantizerConfig,
              cache: BufferCache) -> OLMoBlock:
        if config.block_type == BlockType.sequential:
            return OLMoSequentialBlock(layer_id, config, q_cfg, cache)
        elif config.block_type == BlockType.llama:
            return OLMoLlamaBlock(layer_id, config, q_cfg, cache)
        else:
...
class OLMoSequentialBlock(OLMoBlock):
    ...
    def __init__(self, layer_id: int, config: ModelConfig, 
                 q_cfg: NVFP4_QuantizerConfig, 
                 cache: BufferCache):
        super().__init__(layer_id, config, q_cfg, cache)
        ...
        ) if not q_cfg.is_layer_enabled("attn_qkv") else \
        NVFP4_Linear_Type(q_cfg)(
            config.d_model, sum(self.fused_dims), bias=config.include_bias, device=config.init_device,
            q_cfg=q_cfg, layer_type="attn_qkv"
        )
        
        # Feed-forward input projection.
        self.ff_proj = nn.Linear(
            config.d_model, self.hidden_size, bias=config.include_bias, device=config.init_device
        ) if not q_cfg.is_layer_enabled("mlp_fc1") else \
        NVFP4_Linear_Type(q_cfg)(
            config.d_model, self.hidden_size, bias=config.include_bias, device=config.init_device,
            q_cfg=q_cfg, layer_type="mlp_fc1"
        )
...
class OLMo(nn.Module):
    def __init__(self, config: ModelConfig, q_cfg: NVFP4_QuantizerConfig | None, init_params: bool = True):
        super().__init__()
        self.config = config
        ...
        blocks = [OLMoBlock.build(i, config, q_cfg, self.__cache) for i in range(config.n_layers)]
