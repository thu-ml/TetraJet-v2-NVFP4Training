...
from olmo.quantization_real import linear_mixprec_record, linear_oscillation_reset_with_mix
...
class Trainer:
    ...
    def model_forward(
        ...
        if batch["input_ids"].max() > self.cfg.model.embedding_size:
            log.warning(
                'Encounter Bad input_ids : batch["input_ids"].max() > self.cfg.model.embedding_size'
            )
            ban_mask = batch["input_ids"].max(dim=1).values > self.cfg.model.embedding_size 
            batch["input_ids"][ban_mask] = 0
            batch["instance_mask"][ban_mask] = False
        ...
        log.info(
            ...
            + "\n".join(
                [
                    ...
                    or name.startswith("optim/learning_rate_group")
                ]
            )
        )
        ...
        with torch_profiler as p:
            for epoch in range(self.epoch or 0, self.max_epochs):
                ...
                for batch in self.train_loader:
                    ...
                    # ========== Hack here for Quantized-Training ======
                    if self.cfg.quant_nvfp4.enable_nvfp4_training:
                        linear_mixprec_record(self.cfg.quant_nvfp4,
                                              self.global_step - 1,
                                              self.dist_model)

                        linear_oscillation_reset_with_mix(self.cfg.quant_nvfp4, 
                                                self.global_step - 1,
                                                self.dist_model)
                        dist.barrier()
                    # ==================================================
